...
namespace trabalhoPOOList.Tests
{
    ...
    public class ReservaCinquentaTests
    {
        ...
        private ReservaCinquenta CriarReservaParaTeste(DadosReserva dadosMock = null)
        {
            var reserva = (ReservaCinquenta)FormatterServices.GetUninitializedObject(typeof(ReservaCinquenta));

            var repoField = typeof(ReservaCinquenta).GetField("repo",
                BindingFlags.NonPublic | BindingFlags.Instance);
            repoField?.SetValue(reserva, new RepositorioMock { DadosFicticios = dadosMock });

            reserva.Espaco = new EspacoCinquenta();
        ...
        [TestMethod()]
        public void Construtor_DeveInicializarComEspacoCinquenta()
        {
            var reserva = CriarReservaParaTeste();

            Assert.IsInstanceOfType(reserva.Espaco, typeof(EspacoCinquenta));
            Assert.AreEqual(50, reserva.Espaco.CapacidadeDoEspaco);
        }

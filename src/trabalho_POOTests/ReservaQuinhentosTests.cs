...
{
    [TestClass()]
    public class ReservaQuinhentosTests
    {
        ...
        private ReservaQuinhentos CriarReservaParaTeste(DadosReserva dadosMock = null)
        {
            var reserva = (ReservaQuinhentos)FormatterServices.GetUninitializedObject(typeof(ReservaQuinhentos));

            var repoField = typeof(ReservaQuinhentos).GetField("repo",
                BindingFlags.NonPublic | BindingFlags.Instance);
            repoField?.SetValue(reserva, new RepositorioMock { DadosFicticios = dadosMock });

            reserva.Espaco = new EspacoQuinhentos();
        ...
        public void Construtor_DeveInicializarComEspacoQuinhentos()
        {
            ...
            Assert.IsInstanceOfType(reserva.Espaco, typeof(EspacoQuinhentos));
            Assert.AreEqual(500, reserva.Espaco.CapacidadeDoEspaco);
        }
